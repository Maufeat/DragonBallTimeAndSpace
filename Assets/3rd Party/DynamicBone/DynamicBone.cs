...
public class DynamicBone : MonoBehaviour
{
    ...
    public void FreezeAllParticles(float strength)
    {
        ...
        {
            particle.m_PrevPosition = Vector3.LerpUnclamped(particle.m_PrevPosition, particle.m_Position, strength);
        }
    }
    ...
    //master function call by GameDirector
    ...
    void UpdateDynamicBones(float t, float updateRate)
    {
        if (m_Root == null)
            return;
        ...
        for (int i = 1; i < m_Particles.Count; ++i)
        {
            ...
            if (m_Colliders != null)
            {
                ...
                for (int j = 0; j < m_Colliders.Count; ++j)
                {
                    ...
                    if (c != null && c.enabled)
                        c.Collide(p.m_ParentIndex, ref p.m_Position, particleRadius);
                }
            }
