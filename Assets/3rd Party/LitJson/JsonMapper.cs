    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            foreach (FieldInfo fieldInfo in type.GetFields())
            {
                ...
                propertyMetadata.Info = fieldInfo;
                propertyMetadata.IsField = true;
                propertyMetadata.Type = fieldInfo.FieldType;
                try
                {
                    value.Properties.Add(fieldInfo.Name, propertyMetadata);
                }
                catch (Exception)
                {
                    FFDebug.LogError("JsonMapper", "json parse error ------------------------- f_info.Name = " + fieldInfo.Name + ",   p_data = " + propertyMetadata.ToString());
                }
            }
        ...
        private static object ReadValue(Type inst_type, JsonReader reader)
        {
            ...
            {
                if (reader.Token != JsonToken.Float && reader.Token != JsonToken.Int && reader.Token != JsonToken.Long && reader.Token != JsonToken.String && reader.Token != JsonToken.Boolean)
                {
                    object obj = null;
                    if (reader.Token == JsonToken.ArrayStart)
                    {
                        ...
                        for (; ; )
                        {
                            value = JsonMapper.ReadValue(elementType, reader);
                            if (reader.Token == JsonToken.ArrayEnd)
                            {
                                break;
                            }
                ...
                {
                    reader.Value = float.Parse(reader.Value.ToString());
                    type = typeof(float);
                }
        ...
        private static IJsonWrapper ReadValue(WrapperFactory factory, JsonReader reader)
        {
            ...
            {
                jsonWrapper.SetDouble(float.Parse(reader.Value.ToString()));
                return jsonWrapper;
            }
