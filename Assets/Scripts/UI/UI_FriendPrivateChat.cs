...
public class UI_FriendPrivateChat : UIPanelBase
{
    ...
    public override void OnInit(Transform _root)
    {
        ...
        this.lbl_input.onValueChanged.AddListener(new UnityAction<string>(this.OnValueChanged));
    ...
    {
        UIManager.Instance.DeleteUI<UI_FriendPrivateChat>();
    }
    ...
    private void btn_zhen_on_click(PointerEventData eventData)
    {
        ...
        this.friendController.SendChat(this.mCurChatTargetId, "对方给您发送了一次窗口抖动", 0U, null, true);
        this.friendController.OnReceiveChatData(this.GetSelfChatData("您给对方发送了一次窗口抖动", true));
    }
    ...
    private void btn_send_on_click(PointerEventData eventData)
    {
        ...
        this.friendController.SendChat(this.mCurChatTargetId, text, 0U, null, false);
        this.friendController.OnReceiveChatData(this.GetSelfChatData(text, false));
    ...
    public void AddPrivateChatFriend(relation_item frienddata)
    {
        ...
        this.friendController.ReadedPrivateChatLocalData(frienddata.relationid);
        if (this.friendController.JudgeNewFriend(frienddata.relationid.ToString()))
        {
            this.friendController.AddNewFriend(frienddata.relationid.ToString());
        }
        else
        {
            this.friendController.RefreshFriendTip();
        }
    }
    ...
    private void SetupChatPanel()
    {
        ...
        if (this.friendController.chatDataDic.ContainsKey(relation_item.relationid.ToString()))
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (!(myChatData.charid != MainPlayer.Self.GetCharID().ToString()) || !(myChatData.tocharid != MainPlayer.Self.GetCharID().ToString()))
                {
                    bool flag = false;
    ...
    private GameObject CreateChatItem(MyChatData chatData)
    {
        ...
        ManagerCenter.Instance.GetManager<UITextureMgr>().GetTexture(ImageType.OTHERS, imgname, delegate (UITextureAsset asset)
        {
            if (asset == null)
            {
                FFDebug.LogWarning("UI_FriendPrivateChat", "  req  texture   is  null ");
                return;
            }
            if (imgicon == null)
            {
